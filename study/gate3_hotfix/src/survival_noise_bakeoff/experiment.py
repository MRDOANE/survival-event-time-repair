...
def execute_trial(
    ...
    if hotfix is not None and spec.dataset["name"] != hotfix["affected_dataset"]:
        raise RuntimeError(
            "the Gate 3 hotfix is resume-only and refuses to execute a "
            f"non-GBSG2 trial: {spec.trial_id}"
        )
