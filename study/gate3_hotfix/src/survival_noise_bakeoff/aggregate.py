...
def _hotfix_integrity(
    ...
    affected = [
        row
        for row in trials
        if row.get("dataset", {}).get("name") == GATE3_HOTFIX_DATASET
    ]
    preserved = [
        row
        for row in trials
    ...
    if baseline_path.is_file():
        ...
        baseline_valid = bool(
            ...
            and baseline.get("base_scientific_config_hash")
            == GATE3_BASE_SCIENTIFIC_CONFIG_HASH
            and baseline.get("prior_completed_trials") == 960
            and baseline.get("prior_error_trials") == 240
        )
    affected_valid = all(
        row.get("status") == "complete"
        and row.get("compatibility_hotfix_id") == GATE3_HOTFIX_ID
        and row.get("execution_implementation_sha256") == current_implementation
        for row in affected
    )
    preserved_valid = all(
        row.get("status") == "complete" and row.get("compatibility_hotfix_id") is None
        for row in preserved
    )
    passed = bool(
        baseline_valid
        and len(affected) == 240
        and len(preserved) == 960
        and affected_valid
        and preserved_valid
    )
    return {
        ...
        "preserved_trial_count": len(preserved),
        "affected_trials_bound_to_hotfix": affected_valid,
        "preserved_trials_untouched": preserved_valid,
        "baseline_audit_valid": baseline_valid,
        "execution_implementation_sha256": current_implementation,
    }
...
def aggregate_results(output: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    hotfix_integrity = _hotfix_integrity(trials, config, output)
    completeness["hotfix_integrity"] = hotfix_integrity
    completeness["complete"] = bool(
        completeness["complete"] and hotfix_integrity["pass"]
    )
