...
def _load_public_covariates(
    ...
    try:
        ...
        if source == "rossi":
            ...
            # Reference coding preserves the categorical model space while keeping
            # the intercept-augmented survreg design matrix full rank.  Gate 3's
            # original all-level coding created three exact aliases in GBSG2.
            feature_frame = pd.get_dummies(feature_frame, drop_first=True, dtype=float)
