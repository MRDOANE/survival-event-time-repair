...
run_check <- function() {
  ...
  # Mirror the GBSG2 reference-coded structure that the first Gate 3 run
  # exposed as rank deficient under all-level dummy coding.
  n.rank <- 260
  group.a <- sample.int(2L, n.rank, replace = TRUE)
  group.b <- sample.int(2L, n.rank, replace = TRUE)
  group.c <- sample.int(3L, n.rank, replace = TRUE)
  rank.data <- data.frame(
    X1 = rnorm(n.rank),
    X2 = rnorm(n.rank),
    X3 = rnorm(n.rank),
    X4 = rnorm(n.rank),
    X5 = rnorm(n.rank),
    X6 = as.numeric(group.a == 2L),
    X7 = as.numeric(group.b == 2L),
    X8 = as.numeric(group.c == 2L),
    X9 = as.numeric(group.c == 3L)
  )
  rank.design <- stats::model.matrix(~ ., data = rank.data)
  if (qr(rank.design)$rank != ncol(rank.design)) {
    stop("reference-coded preflight design is not full rank")
  }
  rank.event <- exp(1.3 + 0.2 * rank.data$X1 + rnorm(n.rank, sd = 0.5))
  rank.censor <- exp(1.6 + rnorm(n.rank, sd = 0.7))
  rank.status <- as.integer(rank.event <= rank.censor)
  rank.time <- pmin(rank.event, rank.censor)
  rank.interval <- rank.data
  rank.interval$time_lower <- rank.time
  rank.interval$time_upper <- NA_real_
  rank.interval$time_upper[rank.status == 1L] <-
    ceiling(rank.time[rank.status == 1L] / 0.7) * 0.7
  rank.interval$time_lower[rank.status == 1L] <- pmax(
    rank.interval$time_upper[rank.status == 1L] - 0.7,
    1e-6
  )
  rank.interval$status <- rank.status
  rank.interval <- rank.interval[, c(
    "time_lower", "time_upper", "status", paste0("X", 1:9)
  )]
  rank.model <- IntervalSurvregModelWrapper$new(dist = "lognormal")
  rank.model$fit_interval(rank.interval)
  rank.imputed <- rank.model$conditional_median_times(rank.interval)
  if (
    any(!is.finite(rank.imputed)) ||
      any(rank.imputed < rank.interval$time_lower - 1e-10) ||
      any(
        rank.status == 1L &
          rank.imputed > rank.interval$time_upper + 1e-10
      )
  ) {
    stop("reference-coded conditional imputation preflight failed")
  }
  list(
    status = "pass",
    ...
    reference_coded_design_rank = qr(rank.design)$rank,
    reference_coded_design_columns = ncol(rank.design),
    reference_coded_imputation_finite = all(is.finite(rank.imputed))
  )
}
