...
IntervalSurvregModelWrapper <- R6::R6Class(
  ...
  public = list(
    ...
    fit_interval = function(data) {
      ...
      coefficients <- as.numeric(stats::coef(self$model))
      if (
        any(!is.finite(coefficients)) ||
          length(self$model$scale) != 1L ||
          !is.finite(self$model$scale) ||
          self$model$scale <= 0
      ) {
        stop(
          "interval model fit is non-identifiable; use a full-rank covariate encoding"
        )
      }
      finite_times <- c(
    ...
    conditional_median_times = function(interval_data) {
      ...
      if (any(event)) {
        ...
        if (any(!is.finite(location)) || !is.finite(scale) || scale <= 0) {
          stop("conditional interval imputation received an invalid fitted distribution")
        }
